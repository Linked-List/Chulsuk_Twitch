...
function onMessageHandler(target, context, msg, self) {
  ...
  if(commandName === '!출석시작' && context.username === ignore_list[0] && chulSuk === false) {
    // 방장만 가능한 명령어
    chulSuk = true;
    user_list = [];
    ...
    smtpTransport.sendMail(mailOpt, function(err, res) {
      if (err) {
        ...
        client.say(
          target,
          '띠용 뭔가 잘못되고있어 ㅠ_ㅠ 대충 백업은 했는데 링크드불러바'
        );
  ...
  } else if (chulSuk === true && ignore_list.indexOf(context.username) === -1) {
    // ignore_list에 없는 유저만 읽기
